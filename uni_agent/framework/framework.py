        ...
        *,
        ...
        expected_reward_fields = (
            (
                ...
                deepcopy((trajectories[-1].extra_fields or {}).get("dsh_reward_info")),
            )
            if trajectories
            else (None, None, {}, None)
        )
        ...
        if any(
            (
                trajectory.finished,
                trajectory.reward_score,
                trajectory.reward_metrics,
                (trajectory.extra_fields or {}).get("dsh_reward_info"),
            )
            != expected_reward_fields
        ...
        *,
        ...
        strict_unfinished_episodes = (
            ...
            if self._require_finished_episode
                ...
                any(trajectory.finished is False for trajectory in trajectories)
                for _, trajectories, _ in successful_outcomes
            )
        )
    ...
    async def _run_agent_episode(
        ...
        *,
        ...
        async with _log_scope(parent_log):
            ...
            trusted_result = build_reward_info(task_result)
            if self._require_verifier_reward and task_result.verifier_reward is None:
                raise ValueError("require_verifier_reward requires verifier_reward")
            if (
                self._require_verifier_reward
                and self.reward_loop_worker_handles
                and self._custom_reward_function_configured
            ):
                raise ValueError("strict verifier reward cannot be replaced by a custom reward worker")
            if session_trajectories:
                ...
                session_trajectories = [
                    replace(
                        ...
                        extra_fields={
                            **(trajectory.extra_fields or {}),
                            **(
                                {"dsh_reward_info": deepcopy(trusted_result)}
                                if task_result.reward_info is not None or task_result.verifier_reward is not None
                                else {}
                            ),
                        },
                    )
