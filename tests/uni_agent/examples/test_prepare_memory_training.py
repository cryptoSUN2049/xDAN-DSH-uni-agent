...
def test_actual_shell_hydra_and_native_from_config(inputs, tmp_path, mode, family, course_id):
    ...
    assert config.transfer_queue.backend.SimpleStorage.num_data_storage_units == (
        1 if course_id == "work-state-memory-core-v1" else 8
    )
...
@pytest.mark.parametrize("change", ["remove", "override", "previous-two"])
def test_core_storage_cpu_budget_cannot_drift(inputs, change):
    ...
    if change == "remove":
        ...
        manifest["command"].append(key + ("=2" if change == "previous-two" else "=8"))
    path = inputs["output_dir"] / "manifest.json"
    path.write_text(json.dumps(manifest))
