...
## 2026-09-22：MOPD 后端与算法选择

- 用户明确选择 VERL 以方便调试和修改，后续实现以该路线为准；Tinker 已有离线代码保留作参考，不继续提交其云端任务。
- 资产复用成本与正式训练后端优劣分开判断，不以已投入代码形成路径依赖。
- MOPD 接入优先复用原生 teacher_models / teacher_key；业务数据来源和教师领域使用独立字段。原生 k1/PPO 与 rollout-fixed IS 的概率版本、裁剪和归约不同，先写清精确目标再比较效果。
