...
沿用紧随其后的G0—G6。520训练/160公开开发已落盘，统一857项回归通过；提交发布、Linux canary与新版GPU运行尚未完成。初始16步是诊断，不代表全课程已消费或充分训练。普通任务失败不终止整套评估；轨迹/奖励身份、数值或源版本损坏必须阻断相应作业。
...
- [ ] G1 统一回归、自review、Ruff双门、commit/push固定部署SHA。
- [ ] G2 新GPU checkout、固定DSH/VERL与原venv核验、真实Linux canary；检查GPU占用后启动。
...
平台get_goal在2026-09-10核查为paused；本文件已刷新，不等于平台后台goal已恢复。工具不支持修改现有objective或解除paused，需客户端恢复操作。
