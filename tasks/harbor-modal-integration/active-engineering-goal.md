...
沿用紧随其后的G0—G6。520训练/160公开开发已落盘，统一857项回归通过；已发布至b4d04d8，Linux canary 8项通过；新版GPU r3模型初始化通过但外部DSH任务因CPU预算不足未执行，已受控停止；已由511bd71固定core存储1、95项测试通过并push；新r4已部署发起，待真实采样/更新验收。初始16步是诊断，不代表全课程已消费或充分训练。普通任务失败不终止整套评估；轨迹/奖励身份、数值或源版本损坏必须阻断相应作业。
