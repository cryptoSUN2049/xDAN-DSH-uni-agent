...
本清单用于加快确认完整流程，不另设一套目标，也不替代下面W0—W6。当前r4已到2/8步，后面三项尚未验收；即时进度及原始证据以handoff与run报告为准。

- [x] E1 固定部署核验：DSH SDK/runtime 0.1.3a2与二进制hash匹配，集成源码、有效VERL基线及显式补丁、模型/任务/配置身份绑定已核验，复用现有环境。
- [ ] E2 本课程既定8步执行完成：按n4完整组实际采样、评分和训练计算，审计完整组唯一消费，无未知/重复消费；合法零奖励不主动停止，安全与证据拒绝仍按原合同处理。
- [ ] E3 Checkpoint完整保存：核step4/8工件完整性及源码、模型、任务/verifier、配置、实际策略版本与母实验谱系绑定；文件存在不等于有效学习。
- [ ] E4 独立reload与fresh评估：新进程加载本课程母checkpoint，核加载身份与母checkpoint未改；覆盖固定评估任务，逐题如实记录成功、合法低分或拒绝/失败，保留原回执、实际消费与审计结果，不用启动或尝试评估冒称严格验收通过。
...
- [ ] W2 数据与真实基线：发布固定摘要的训练/公开开发清单，分别统计结构族、独立实例、采样与实际消费；真实学生在同预算下逐族执行。全零/全满分时先诊断奖励可辨性，不伪造差异。
- [ ] W3 本课程在线RL：同策略n4完整组、A/B及sibling隔离、真实token/logprob/version与原回执匹配；无未知/重复消费，拒绝组不进入更新，合法低分组不因低分被筛掉。
- [ ] W4 有效更新与保存：至少存在可归因于本课程非零优势的有限非零梯度及实际参数更新，冻结base正确，optimizer推进，checkpoint完整保存并绑定版本/数据/配置摘要；报告零梯度步和实际覆盖。
- [ ] W5 独立reload与结果复跑：新进程加载该母实验checkpoint，新session完成固定开发任务并产出可复核评分；核对加载身份、无意外更新/改写母checkpoint、消费与评分一致。逐族记录成功与失败，不要求训练后必然提分。
- [ ] W6 交付：可手动重跑的prepare/train/audit/reload命令、运行顺序与故障处理.md、证据清单/摘要、goal/handoff更新，Ruff双门与相关测试通过后commit/push。
