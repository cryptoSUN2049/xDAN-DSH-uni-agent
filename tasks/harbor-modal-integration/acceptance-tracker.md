...
| G1.2 M1 DSH在线RL | 真实token/trace/fresh receipt绑定；正确准入；有限非零梯度；LoRA数值变化、base冻结；独立reload及留出eval | 未完成；v2-r4启动中。v1零更新，v2-r1/r2存储失败，r3启动前失败 |
...
- [ ] r4自然退出：supervisor exit_code=0，reason=training-exited；训练manifest completed。
- [ ] 审计训练组真实消费、奖励方差；基础设施错误与可信0分区分，无未匹配消费。
- [ ] loss/gradient有限，存在非零更新；不能用文件hash变化替代逐张量检查。
- [ ] checkpoint逐张量有限、LoRA变化、base不变；optimizer状态有限且step/动量有真实更新证据。
