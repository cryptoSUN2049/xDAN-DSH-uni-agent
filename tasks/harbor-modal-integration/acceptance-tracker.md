...
| G1.2 M1 DSH在线RL | 真实token/trace/fresh receipt绑定；正确准入；有限非零梯度；LoRA数值变化、base冻结；独立reload及留出eval | 未完成；v2-r4两步更新及消费/数值审计通过，独立reload两条fresh评估通过，GPU释放。v1零更新，v2-r1/r2存储失败，r3启动前失败 |
