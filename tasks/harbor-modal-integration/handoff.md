...
- **Harbor学生VAL_ONLY r1已中止**：代码f097970，GPU /root/runs/t2-harbor-student-r1；313秒后controller-health-failed/exit -6，Mac SSH keepalive超时→control-unavailable，任务尚未开始，无评分/更新。GPU进程退出且显存空。controller cleanup_errors=[]但docker_cleanup_proven=false，不追认清理完成。日志归档/workspace/reports/t2-harbor-student-r1-startup-failure.tar.gz，SHA02e8e0a9acfcb25605ce42bcb0c4755299c934d4c10895d71630351fb9340fe9。继续独立M1 redact_email原样4/2课程，同时补隧道失败诊断。
...
- **T2 Harbor scripted真实4正反例通过**：`/private/tmp/harbor-t2-scripted-r1`，positive1/wrongoutput0/noop0/tamper拒绝无reward，全部cleanup_confirmed。报告t2-harbor-scripted-r1-result.json；主进程完整Harbor环境257tests通过。双镜像显式pin的r2四mode再次全部通过（t2-harbor-scripted-r2-result.json），新学生部署用/private/tmp/harbor-t2-package-r2/task，不能再用r1TaskRef。固定T2 agent image b016c85140a58f7d842eadb0238925ee1c347143cc7bede5b9b35bfa38747dca（父846b46），verifier2d504ede65261caa1b16432508d20752c36f8df873b0e3ddf482ed11017c27fb；两镜像私有Release harbor-t2-dsh-0.1.3a2-b016c851已发布并回下载hash通过。新版学生→Gateway→Harbor→训练侧准入尚未执行；不算RL训练完成。
