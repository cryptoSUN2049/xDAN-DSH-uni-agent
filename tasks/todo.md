...
- [x] 用户明确“开始处理数据 / 好的 开展起来”，实施设计门已通过。
- [x] 源解析与监督合同单测；逐教师/语言/工具质量过滤单测；后台成功/失败/不可覆盖测试共84项通过。
- [ ] 全量真实处理，统计保留/排除/隔离与版本血缘。
- [ ] 完整清洗池与三教师子池；20K缺口检查和精确监督token。
- [ ] 私有HF发布并回读；ms-swift模型模板与小步GPU验收；正式SFT。

## 原版台账与分布交付（2026-09-24）

- [x] 13个唯一仓库原版HF路径、固定revision、原文件清单和README hash。
- [x] 原版数量/分布与实测结果分列；明确文件/前缀/轨迹/任务单位。
- [x] 五个近期指定库的主训练文件下载；四库详细审计证据。
- [x] 目标能力与供给缺口对比，20K建议比例独立成层。
- [ ] 实施审批及20K来源选择答复后，推进正式处理、HF发布和训练。

Review：无训练ready或上传完成声明；原始台账保留所有指定源，即使内容需隔离。

## APUS完整数据与20K SFT（2026-09-24）

- [x] 读取交接、检查工作树、HF账号与GPU/环境入口。
- [x] 形成sft20k-execution-design.md具体设计。
- [ ] 设计审阅确认（AGENTS Human Gate）；等待期间可进行授权的原始下载及只读审计。
- [ ] 全量来源处理、Gemini排除、跨源去重和分域缺口报告。
- [ ] 冻结真实20K配方、监督mask、独立dev与token预算。
- [ ] 上传完整/20K私有HF并回读验收。
- [ ] GPU小步验收、启动正式训练、持续报告。

## CodeFlame独立过滤（2026-09-24）

- [x] 最终清单加入排除Gemini 3.1、未知隔离、轨迹组检查与统计守恒规则。
- [ ] 固定原始文件并执行全量身份/来源审计，产出独立过滤版本。
- [ ] 验收过滤后教师分布、任务数、token与排除清单，再冻结训练配额。

Review：本轮为文档约束更新，未执行数据过滤或训练。

# 当前最终数据清单v3

- [x] Fable/GPT5.6核心及通用保留要求写入最终训练数据清单.md。
- [x] 1000目标来源/领域/监督/组件/验收清单整合；520现成+480新生。
- [ ] 全量准入与真实教师执行待建设，不把配额当ready。

# 当前配方v2（2026-09-24）

- [x] Fable+GPT5.6核心400、专项600目标与中立数据结构落盘，v1.1降为历史。
- [ ] 核心400独立任务真实准入/数量/token审计，不能将目标当ready。

# 强教师基础数据复核（2026-09-24）

- [x] 纠正Fable原始/独立来源遗漏，画像评级冲突解释。
- [x] GPT5.6官方卡/本地切片与GPT6候选搜索；更新1K条件配额。
- [ ] 完整文件按身份/session/工具配对/许可/污染审计；冻结可用数量与目标mask。

Review：强教师基础恢复优先，不因品牌放行坏数据；440/560是目标，不是已验收成果。
...
- [x] 数据三档准入、20种子任务规格与20→100→1K放行条件落盘。
- [ ] 五个锚点任务输入/环境/正确错误空控制建设。
- [ ] 教师接口与硬预算核定，真实执行并记录成本。

Review：仅规格，未生成任务fixture或可训练轨迹；不要把任务清单记为20题验收完成。

# APUS Tool Call专项（2026-09-24）

- [x] apus-chat-v1-tool-call.md落盘，格式/合同/迁移/测试完整分层。
- [x] 接入HTML与项目索引、交接。
- [ ] 后续实现：冻结v1范围和golden样例，E1适配及真实回归；当前没有parser实现或新训练。

Review：本次交付是专项设计，不宣称API兼容或模型格式迁移已完成。

# Agent SFT 数据工厂方案（2026-09-24）

- [x] 解析用户数据画像；核对候选、小样与完整库存边界。
- [x] 当前HF卡与Curator官方文档核验；记录模拟工具、版本日期与许可风险。
- [x] 扩展MiMo专题：生产工具、轨迹合同、Qwen/Apus迁移、多harness验收；设计落盘。
- [ ] 后续：确认Apus规格与GPT6-sol API身份/预算；数据准入与20任务试点。

Review：目前是设计成果；没有凭模型名/画像分级自动放行，未发起计费请求。

# MiMo Agent SFT 9B 专题（2026-09-24）

- [x] 核对论文§7表4–7及固定模型revision，保存来源快照。
- [x] 独立HTML专题：算法/数据/环境/评测/未知项/我们的路线，接入产线入口。
- [x] 浏览器验证桌面/手机无横向溢出、目录筛选、结果列切换、锚点；来源独立复核。
- [ ] 后续：原版与MiMo起点同环境评测；当前未下载模型或启动新训练。

Review：SFT是监督蒸馏，不是OPD。保留其他会话未提交改动。
...
## 2026-09-09 当前效率收尾
- [x] selected失败归因与原始证据云盘归档回读。
- [x] 独立四题评估控制器：失败隔离、真实消费、全量汇总；19项CPU及3项Linux追加通过。
- [x] CPU/Ruff/commit/push/固定部署9dea127；GPU四题全部终态，3题真实消费通过，WS06 max-tokens失败保留，收尾/母CK/清理通过。
- [x] 本课程32条B零奖励业务原因独立核验；短课程设计已落盘，未实现/训练。
- [x] 合并操作指南与最新goal/handoff，保留旧失败；HTML1440/390视觉检查通过。
- [ ] 后续W4：真实非零梯度/参数更新，不能将工程收尾冒充学习。

## 2026-09-09 继续补齐真实有效更新：独立短课程
...
## 短课程之后：继续原生四能力（2026-09-09）

- [x] 保存短课程有效更新、独立 reload、云盘归档及视觉证据，提交并推送 7e8ce46。
- [x] 明确旧执行 checkout 与修复后离线 auditor 的职责，修正人工复跑指南。
- [x] RSI 入口适配已经锁定的 VERL finish-reason overlay；207项回归及Ruff通过，b1c568b已推送并在GPU隔离部署，未知源码仍拒绝。
- [x] 固定b1c568b并完成RSI H0两类真实开发任务；2条TQ唯一读回，raw-token审计passed，均合法reward0；不冒称RL或晋升。
- [x] 审计短课程到真实多文件工作状态课程的初始化路径：当前入口不支持warm-start，r16/alpha16工件已实查；新初始化合同尚待实现，未放宽reload同课程门。
- [ ] 继续 N2/N3/N4、结果复跑与封存评估；短课程完成不关闭总 goal。

## 核心记忆/context能力主线（最新优先）

- [x] r2工程重复与独立reload完成；32终态B全满分，零更新结论保留。
- [x] 四类数据方向与异步阻断审计记录，停止异步代码实施与误发面板需求。
- [ ] 归因已有WS01/03/05失败，保留WS06无需保存负例。
- [ ] 冻结核心任务基线与评分，复用真实A→冻结→新B执行，先不依赖跨课程初始化。
- [ ] 依据错误定向扩实例/难度，再在线训练，核有效更新与独立reload。
- [ ] 同预算未训练实例评估与基线对照，保存阶段报告。

Review：不将144条预算当已生成数据，不把固定权重评估计作RL；异步作为后续性能节点。

## 最新goal分阶段（2026-09-10）

- [x] 刷新总目标为P1工程→P2效果→P3真实上下文与跨场景，避免相互替代。
- [ ] 立即完成P1 G1—G6：统一857项已通过，尚需发布/部署/canary/GPU/独立reload与归档。
- [ ] P2冻结独立评估及记忆消融；公开160dev不当封存集。
...
## 2026-09-10 reader 配对诊断（用户明确批准）
- [x] 保持旧入口行为，冻结一次并准备隔离reader分支。
- [x] 独立诊断证据审计：可信失败保留，篡改拒绝，不改变训练准入。
- [x] 有界执行入口：每题1A、每条件4B、无TQ消费，记录固定身份与配对分母。
