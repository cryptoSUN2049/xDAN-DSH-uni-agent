...
> **决定（2026-09-18，用户）：继续使用 Modal，暂不迁移。** 理由：RunPod Pod 官方不支持自建 Docker（已查证并实测）；新租一台 Docker 虚拟机的价格与修好泄漏后的 Modal 同一量级（并发 16 约每小时 1 美元），还要额外承担镜像缓存、磁盘、清理和网络的运维。本简报保留，待出现"已付费且闲置的 Docker 机器"或"支持特权容器、训练与沙箱同机的 GPU 平台"时再启用。
...
4. **继续用 Modal，但把泄漏堵住**。如果清理脚本加优雅停止能把实际成本压到每条约 0.047 美元（Sandbox 单价，1 核 2 GB、15 分钟），一轮 20 步约 30 美元，可能已经够用，不必迁移。（初版写的 0.016 美元、12 美元用的是 Function 单价，偏低约 3 倍。）这条要作为基线一起测。
