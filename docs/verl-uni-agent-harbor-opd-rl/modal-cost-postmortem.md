...
1. **沙箱泄漏（主因）**。Harbor 创建 Modal 沙箱时写死 `sandbox_timeout_secs=86400`、`sandbox_idle_timeout_secs=None`（`harbor/environments/modal.py:881`）。Harbor CLI 进程被杀时不清理沙箱，于是沙箱继续计费，最长 24 小时。命令行没有暴露这两个参数。
...
| 残留沙箱清理脚本 | `deployment/bootstrap/modal-sandbox-cleanup.sh` | 杀完训练必须跑一次；`--older-than` 保护在跑的 trial |
