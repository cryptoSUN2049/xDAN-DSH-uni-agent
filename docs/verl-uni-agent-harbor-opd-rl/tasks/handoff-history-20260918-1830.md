...
更新：2026-09-18 18:30 UTC。历史逐轮记录在 `handoff-history.md`，持久决策与每轮记录在 `memory.md`（按时间追加），踩坑在 `lessons.md`（46 条），任务清单与每轮计划在 `todo.md`（最新是阶段 H5），成本复盘在 `../modal-cost-postmortem.md`。
...
- **最近一轮：pipe-r11 已完成，验收 PASS**（2026-09-18 18:23 UTC）。成本通过（26.83 美元，计费/实用 1.02，残留 0）；机制通过；学习信号在训练集与 8 道验证题上看不出提升；长度无副作用。报告 `../pipe-r11/report.md`。双卡 pod 空闲。
...
- **下一步**：在共享评估集 eval-set-v1（78 道）上配对评测基座与 pipe-r11 最终模型（待负责人确认）；pipe-r10 对照组待单卡 pod 恢复。下一轮起启用"有效 ≥ 4 条即保留"与 VERL 补丁（17b2478）。
- **Modal 额度（10:40 UTC 已调高）**：花费上限 700 美元（已用 374.22），Modal 设定的用量上限 1,000 美元（已用 436.89），费用到 500 美元时扣款、之后上限自动上调；与 Tinker 线共用。
- **待负责人决定**：单卡 pod（12063）需在 RunPod 控制台重启，才能跑对照组；是否把训练拉到 50 步（2 个 epoch，约 20 小时、约 50 美元）。
