...
- [x] pipe-r4 第二次训练：14:41 引擎显存从 45 涨到 91 GiB 后 OOM → 14:48 与 pipe-r7 同步改为 prefix caching 关、单批 4096、显存比例 0.40 后重起
- [ ] 两条跑通后：逐项放开（先开 prefix caching），观察 9B 引擎进程显存，确认增长的根因
