...
接手的时候，pipe-s2 正在阶段 B 中途（12:41 UTC 开始，预计 09-20 08:30–09:30 结束）。孤儿清理和 delta 两个修复 12:47 前已提交并部署，**剩下有时限的是：09-20 01:00 UTC 前让评测会话知道启动 vLLM 时要导出 `KEEP_GPU_PROCESS=1`**（交接文档"分工"一节那条未送达的消息），以及 00:30 前核一次原版评测队列会不会拖过 01:00。阶段 B 在跑的时候不要整体跑 `sync-source.sh`，只换单个文件。
