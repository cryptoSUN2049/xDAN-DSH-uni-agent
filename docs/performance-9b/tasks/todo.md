...
### H7 S1 期间发现、待修
- [x] VERL 文件记录器写到 `src/uni-agent/<project>/<experiment>.jsonl`，且以覆盖方式打开；实验名都是 `pipe-train`，每一轮会覆盖上一轮（pipe-r11 的逐步指标文件已被 S1 覆盖，wandb 与训练日志里仍有）。**已修（e178270）**：S1 续跑必须先修，否则会覆盖第 1–12 步。`40_train.sh` 每次尝试写 `<stage>/metrics-<开始时间>.jsonl`；控制台缺最后一步时改从这些文件合并出 `step-metrics.txt`（后一次尝试优先），`60_resume.sh` 改查 `step-metrics.txt`。S1 第一段已存为 `train/metrics-20260918T192629Z.jsonl`，实测恢复出第 1–12 步。
- [ ] 训练日志里不再出现每步 `step:N - ...` 汇总行（控制台输出被缓冲），监控改读上面的逐步指标文件。
- [ ] **超时轨迹被丢弃而不是记 0 分**：S1 第 2、4 步各有一条轨迹跑满 `trial_timeout_sec=1800`，被包装层强行结束，没写 result.json，于是按会话失败处理（组内有效 ≥ 2 保留其余，VERL 补齐 1 条占位样本）。原因是智能体时限（1800 秒覆盖值）等于整条 trial 时限，智能体用满时间就没有判分时间。后果：拖到超时永远得不到 0 分，系统性少掉一类负面信号，方向上会纵容越跑越久。修法（下一轮，本轮不改）：整条 trial 时限 = 智能体 1800 + 判分 900 = 2700 秒，沙箱寿命 3600 秒；智能体超时后 Harbor 仍对最终状态判分，超时得 0 分。S1 中的发生率约 2%（前约 100 条中 2 条）。
- [ ] **超时强杀会留下沙箱**：`run_harbor_cli` 在超时后直接 `process.kill()`（SIGKILL），Harbor 来不及关自己的沙箱，要等 Modal 空闲 20 分钟回收（S1 在 23:41 UTC 时存活 20 个，超出并发 16）。修法：先发 SIGTERM、给 Harbor 约 60 秒收尾，再 SIGKILL。与上一条一起在下一轮改。S1 截至第 8 步累计 7 条超时强杀，集中在 rdt-609、task_07356 等几道难题，GPU 生成未变慢。
