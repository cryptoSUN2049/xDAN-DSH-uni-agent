...
## 2026-09-19 05:35 OPD 打分实测审计：实现正确，信号主要落在行文风格上
- 用户要求严查"OPD 打分是否有问题"。在 S1 第 12 步 29 条真实轨迹上比对三路：生产配置 vLLM（前缀缓存 + 分块）、关前缀缓存、HF 独立参考。
- 实现层全部通过：id 错位 0；生产与 HF 平均差 0.0074、p99 0.115（bf16 噪声）；前缀缓存与分块不引入偏差；错一位的对照差 0.57，检测灵敏；VERL 切片与 k1 函数和手工推导差 0.0；复算 k1 为 0.0657，日志 0.0611。
- 算法层：
  - k1 有 42.5% 落在 analysis、21.3% 落在 plan，只有 30.6% 在命令上，是否结束任务几乎为 0。OPD 在教行文风格。
  - 句末 `.",` 平均 k1 +0.134，收尾被扣分，文字变长。
  - 第 12 步 6/8 组没有 RL 信号。
  - 详见 `../opd-then-rl-design.md` 第 5 节。
...
## 2026-09-19 09:05 Tinker 分阶段 run 的 75% 节点（来自评测会话 76）；pipe-s2 第 5 步
- Tinker 分阶段 run（OPD 8 步 + RL，快检 58 × 2，与 base-v4 配对）：
  - OPD 刚结束：−0.129。
  - OPD 8 + RL 8：−0.284 [−0.379, −0.190]。
  - OPD 8 + RL 16：−0.267 [−0.362, −0.155]；SWE 与 TL 都显著退步。
  - 与只用 RL 的 A' 同节点比：−0.397 [−0.543, −0.250]。
  - token 中位数是原版的 2.4 倍，跑满轮数 55%；训练 reward 0.16–0.35，A' 同期 0.45–0.77。
  - 结论：OPD 阶段的退步在 16 步 RL 后仍未修复，篇幅膨胀延续到 RL 阶段，奖励变稀疏。
- pipe-s2 第 5 步：解题率 0.613，回复长度 2.20 万，撞上限 3.2%，蒸馏损失 0.084（首次下降）。守护进程在运行。
- 负责人的决定不变：完整跑完。本线阶段 B 重置优化器，能检验"Adam 动量延续"这一解释；如果本线同样追不回来，说明原因在 OPD 造成的风格和篇幅，而不在优化器状态。

## 2026-09-19 Tinker 分阶段 run 100% 全量结果（来自评测会话 76）
- 设置：OPD 8 步 + RL 24 步；全量 78 × 4，按题配对，出错 0。
- 对只用 RL 的 A' final：0.321 对 0.679，差 −0.359 [−0.446, −0.276]，按事先约定的判据为"分阶段更差"；SWE −0.446，TL −0.310。
- 对原版：差 −0.160 [−0.247, −0.077]，显著退步；SWE −0.143，TL −0.170；排除 parse_error 后为 −0.237。
...
## 2026-09-19 14:25 阶段 B 起步；第 13 步零梯度查明；GPU1 队列调整
- 阶段 B 13:00 开始训练，只加载模型：日志只有 `Loaded model from ...pipe-s2-opd/.../global_step_12`，没有加载优化器、rng 或 lr_scheduler。学习率从 3.33e-5 重新预热。
- 每步 2133 / 1840 秒，比阶段 A 慢。按此速度，第 60 步约在 09-20 13:00–16:00。
- **第 13 步 `pg_loss=0`、`grad_norm=0` 不是 bug**：这一批 8 组全是全对或全错，GRPO 优势全为 0。第 14 步恢复（优势 ±0.866，即 4 条中对 2 条）。
  - 第 13–14 步生成的 23 组里有 21 组结果一致：Terminal-Lego 几乎全对，SWE 几乎全错。
  - 阶段 B 的 RL 信号很稀疏，每步通常只有 0–2 组有梯度，这会限制它能追回多少。按负责人决定不改动，只记录。
- 阶段 A 的快检比预计慢：13:09 起，68 分钟完成 34/116，推测是 OPD 模型输出更长、轮数更多。
  - 原队列（TB2.1 原版约 8 小时，然后 eval-set-v1 原版）会把 GPU1 占到约 05:00，挤掉评测会话 5e 01:00 开始的窗口。
  - 14:21 按会话号 1734775 结束旧队列（当时只在等待），换成 `runs/pipe-s2-evalq2.sh`：快检后只跑 eval-set-v1 原版 78 × 4，约 21:00–22:00 结束。
  - TB2.1 原版 89 × 3 挪到 09-20 5e 全量结束之后，在 GPU1 上跑。最终配对结果出来的时间不受影响，本来就要等最终模型。
  - 已通知 5e；`gpu-schedule.md` 已更新。
- 事件监控修正：已有 `train/PASSED` 的阶段不再判"卡住"。此前阶段 A 结束后误报过一次。

## 2026-09-19 14:45 阶段 A（纯 OPD 12 步）快检：显著退步，与联合训练无差别
- 快检 58 × 2，与原版 `qc-base` 按题配对；13:09–14:40，各有 2 条基础设施未完成。证据：`../pipe-s2/quickcheck-phaseA-step12/`。
- **纯 OPD 第 12 步 vs 原版：0.612 → 0.422，差 −0.190 [−0.302, −0.078]，退步**；25 题变差，8 题变好。
  - SWE −0.196 [−0.357, −0.036]，显著。
  - Terminal-Lego −0.183 [−0.350, −0.017]，显著。
- **纯 OPD vs S1 联合第 12 步：−0.017 [−0.112, +0.078]，看不出差别**；SWE +0.036，TL −0.067。
  - 同样 12 步、96 题下，联合训练里的 RL 基本没有贡献，退步来自 OPD（H1 是放大器，不是根因）。
  - 与 Tinker 一致：它那边联合（只蒸馏动作）与纯 OPD 都是 −0.129。
- 行为（原版 → S1 联合 → 纯 OPD）：
  - token 中位数：9,270 → 13,745 → 14,405（纯 OPD 为 +55%）。
