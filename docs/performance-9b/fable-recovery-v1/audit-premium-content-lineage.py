...
for split in ["train", "validation", "test"]:
    for i, row in enumerate(rows(V1 / (split + ".parquet"))):
        ...
        if key:
            lookup[key].append(
                {"split": split, "row": i, "model": row.get("model"), "content_sha256": content_hash(row)}
            )
...
for i, row in enumerate(rows(V2)):
    ...
    candidates = lookup.get(content_hash(row), [])
    exact = [r for r in candidates if r["content_sha256"] == content_hash(row)]
    if not candidates:
        counts["content_unmatched"] += 1
    elif not exact:
        counts["content_hash_collision"] += 1
    else:
        counts["exact_message_content_match"] += 1
            ...
            evidence.append(
                {
                    ...
                    "provenance_status": "exact_message_content_match_not_teacher_authentication",
                }
            )
        else:
...
assert (
    counts["v2_base_rows"]
    == counts["content_unmatched"] + counts["content_hash_collision"] + counts["exact_message_content_match"]
)
report = {
    "counts": dict(counts),
...
(ROOT / "recovery/premium-content-lineage-audit.json").write_text(
