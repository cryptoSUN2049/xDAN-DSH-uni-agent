...
| 思考与蒸馏覆盖 | **学生不思考**（`enable_thinking=False`，轨迹里的 `<think>` 块全空），蒸馏只落在动作 token（JSON 的 analysis/plan/commands），系数 1.0；每 token 值截断 ±10（第 1 步最大 18.7，截断生效）。*更正：本文初版写成"含思考"，是错的* | 学生思考。B'：只蒸馏 `</think>` 之后，系数 1.0；F'：含思考，系数 0.1；不截断 |
| Teacher 打分格式 | 对网关拼出的 token 序列直接打分。**已核实**与 Qwen3.8 原生无思考格式逐字一致，分词器一致（见 `opd-then-rl-design.md` 第 0 节） | 查出格式不一致是嫌疑，修渲染器（`qwen3_5_preserve_thinking`）后重跑 |
...
两线结论不能直接互证：差异落在决定"蒸馏是否有害"的旋钮上（RL/蒸馏相对权重、学生是否思考）。但两线方向一致：VERL S1 在无思考、原生格式、token 对齐的条件下仍显著退步，所以格式和思考 KL 都不是必要条件。VERL 线要回答"OPD 在这里帮不帮忙"，需要一组只改 `TEACHER=0` 的对照；要复现 Tinker 结论，则先对齐 `norm_adv_by_std_in_grpo=False`、同步模式、蒸馏跳过思考（VERL k1 暂无此选项）。
