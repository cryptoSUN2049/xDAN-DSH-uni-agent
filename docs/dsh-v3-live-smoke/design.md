...
日期：2026-09-07。用户已明确允许 git worktree add；隔离 worktree 已创建。CPU 实现已交付并验证聚焦范围，GPU 和付费模型调用未授权。
实现基线：`dsh-adapter / 00520c5`。全局目标仍由 [状态文档](../dsh-adapter/project-status.md) 定义。
...
| `uni_agent/framework/task_runner.py` | 在已解析 Task 上注入 operator-owned episode workdir，复制冻结小文件，reward POST 前复查源/副本 |
| 新增 inference / episode-workspace / auditor 测试 | 验证严格入口、工作目录、身份、回执、token 与 TQ 读回拒绝路径 |
...
## 6. CLI 与证据合同（CPU 已实现，真实执行仍须部署预检）
...
用户原规则禁止手工 `git worktree add`，本轮用户已明确允许这次等价的手工创建方式，
...
用户明确回复“好的 运行 允许用 git worktree add 创建 .Codex/worktrees/dsh-v3-live-smoke”。模型服务与 GPU 实际运行不包含在 CPU 阶段内。
...
实现 `27f7efa` / `3220216`。Task 行不能覆盖 workdir，故在既有 task_runner
resolve 之后增加三个 operator-owned episode 参数；不改 Task 或 verifier 核心。
每个实际 Gateway session 使用新目录、16 个冻结文件；复制前及 reward POST
前检查摘要，auditor 再检查历史 workspace。保护范围仍受第 3 节同 UID 限制。

208 项聚焦测试通过，四模块覆盖率 87%–99%；独立 review 找出的 token
dtype/值、损坏 ZIP、相对 registry 路径问题已修复并通过回归。
全仓本机尝试与已复现的基线依赖失败见 [进度](progress.md) 和
[机器可读验证](verification.json)。不将 M0 CI 绿灯代替 M1 CI。

实际命令、runtime manifest、退出码、产物封存与停止边界见 [runbook](runbook.md)。
GPU watchdog、Linux runtime 恢复与真实 8/8 仍待完成；进程 timeout 未实现 Pod 停费。
