...
the tokenizer/data preflight, then `python -m verl.trainer.main_ppo` with the
validated overrides. The preflight uses the paired VERL tokenizer, processor and
dataset filtering, rejecting fewer effective training rows than one batch or an
empty validation set before starting Ray. It computes the epoch limit from the
requested total steps and effective rows so a small dataset reaches the target
step, where native VERL saves the final checkpoint. `--print-config` only shows
the preliminary composition; use `--preflight-only` to inspect effective counts
and epochs without starting training. Random subset selection requires a fixed
data seed so preflight and trainer filtering select the same examples.

For a bounded one-outer-step check:

```bash
PYTHONPATH=.:verl python -m examples.harbor_opd_rl.launch \
  --mode rl --launch /private/harbor-run/launch.json \
  --total-training-steps 1 --save-freq 1 --preflight-only
```

Remove `--preflight-only` to run. This writes `training-preflight.json` under the
mode's run directory, recording effective row counts, steps, epochs and restore
source. One outer step in separate_async can include multiple optimizer updates;
the step budget is not a count of distinct tasks or proof of nonzero gradients.

Explicit native resume uses an **absolute** target step, not additional steps:

```bash
PYTHONPATH=.:verl python -m examples.harbor_opd_rl.launch \
  --mode rl --launch /private/resume-run/launch.json \
  --resume-from-path /private/harbor-run/rl-training/checkpoints/global_step_5 \
  --total-training-steps 10
```

The source must have `actor/` and `data.pt`, and its step must be below the target.
Native VERL loads model/optimizer/extra and supported queue state; the launcher
does not deserialize optimizer state or certify that GPU restore has succeeded.
Keep the same frozen model, data, controller policy and topology for a continuation.
Step, save and resume options are CLI arguments, not implicit environment variables.
