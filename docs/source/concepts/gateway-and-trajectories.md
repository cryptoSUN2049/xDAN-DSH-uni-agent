...
must preserve finalized `finished`, `reward_score`, `reward_metrics`, and any
DSH proof in `extra_fields["dsh_reward_info"]`, and keep their token arrays
...
### DSH strict typed-result extension

DSH returns bounded `TaskResult.reward_info` evidence and `verifier_reward`; these no longer cross a Gateway reward HTTP endpoint. The Framework validates the result and preserves the proof in `Trajectory.extra_fields["dsh_reward_info"]`. Schema 2 dumps retain a derived `reward_info` projection for historical audit readers. Postprocessors cannot change that proof or finalized result fields.

Strict DSH uses `require_result=True` in the task runner and keeps `require_finished_episode`, `require_verifier_reward`, and all-or-nothing group admission. A custom reward Worker cannot replace a strict verifier score. Generic upstream permissive completion behavior remains separate.
