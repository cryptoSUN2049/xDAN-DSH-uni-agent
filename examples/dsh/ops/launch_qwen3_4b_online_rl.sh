...
printf '%q ' env "CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}" "TRAINER_MODE=${TRAINER_MODE}" "NUM_WARMUP_BATCHES=${NUM_WARMUP_BATCHES}" \
...
run_command=(env "CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}" "TRAINER_MODE=${TRAINER_MODE}" "NUM_WARMUP_BATCHES=${NUM_WARMUP_BATCHES}" \
  "PATH=${PATH}" "PYTHONPATH=${PYTHONPATH}" "PYTHON_BIN=${PYTHON_BIN}" \
  "MODEL_LICENSE_APPROVED=${MODEL_LICENSE_APPROVED}" "LOW_VRAM=${LOW_VRAM}" \
  "MODEL_ID=${MODEL_ID}" "MODEL_PATH=${MODEL_PATH}" "TRAIN_FILE=${TRAIN_FILE}" \
  "TEST_FILE=${TEST_FILE}" "TASK_CONFIG=${TASK_CONFIG}" "RUN_ROOT=${RUN_ROOT}" \
